...
{
	half4 color = tex2D ( diffuseMap, uv );
#ifdef AB_HIGH_PRECISION
...
{
	half4 color = tex2D ( diffuseMap, uv )*tex2D ( maskMap, uv );
#ifdef AB_HIGH_PRECISION
	return  max ( color - threshold, 0 );
#else
	return EncodeColor ( clamp ( color.rgb - threshold.xxx, ( 0 ).xxx, _BloomRange.xxx ) );
...
{
	return half3( DecodeColor ( tex2D ( texMap, uv + dir * distortion.r ) ).r,
		DecodeColor ( tex2D ( texMap, uv + dir * distortion.g ) ).g,
		DecodeColor ( tex2D ( texMap, uv + dir * distortion.b ) ).b );
}
...
inline half4 AnamorphicGlareMat ( float2 uv, sampler2D diffuseMap )
{
	...
	//MatIdx 0
	{
		...
		mainColor = DecodeColor ( tex2D ( diffuseMap, uvOffset ) );
...
inline half4 NineTapGaussian ( float2 uv, sampler2D diffuseMap, float2 stride )
{
	...
	color.rgb += DecodeColor ( tex2D ( diffuseMap, uv ) ) * 0.227027027;

	float2 d1 = stride * 1.3846153846;
	color.rgb += DecodeColor ( tex2D ( diffuseMap, uv + d1 ) ) * 0.3162162162;
	color.rgb += DecodeColor ( tex2D ( diffuseMap, uv - d1 ) ) * 0.3162162162;

	float2 d2 = stride * 3.2307692308;
	color.rgb += DecodeColor ( tex2D ( diffuseMap, uv + d2 ) ) * 0.0702702703;
	color.rgb += DecodeColor ( tex2D ( diffuseMap, uv - d2 ) ) * 0.0702702703;
...
inline half4 FirstPassUpscaleBlurTent ( sampler2D currentMipRT, float2 uvCoords, float2 oneOverTexSize, float BlurRadius )
{
	...
	for ( int i = 0; i < 9; i++ )
	{
		color.rgb += DecodeColor ( tex2D ( currentMipRT, uvCoords + TexelOffsets[ i ] * oneOverTexSize * BlurRadius ) ) * Weights[ i ];
	}
...
inline half4 UpscaleBlurTent ( sampler2D currentMipRT, sampler2D previousUpscale, float2 uvCoords, float2 oneOverTexSize, float BlurRadius )
{
	...
	color.rgb += DecodeColor ( tex2D ( previousUpscale, uvCoords ) );
...
inline half4 DownsampleWithKaris ( float2 texcoord, float2 oneOverTextureSize, sampler2D DiffuseMap )
{
	...
	{
		texels[ i ].rgb = DecodeColor ( tex2D ( DiffuseMap, texcoord + ( TexelOffsets[ i ] * oneOverTextureSize ) ) );
	}
